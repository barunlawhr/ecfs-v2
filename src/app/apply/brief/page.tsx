...
function BriefPage() {
        ...
        const gradeRes = await fetch('/api/grade', {
          ...
          body: JSON.stringify({
            formData: {
              ...
              evidences: evidenceRows.map(r => ({ id: r.id, number: `갑 제${r.서증번호}호증`, name: r.서류명, purpose: '' })),
              hasAgent: false,
              claimType: '',
              sogaType: '',
              soga: '',
  ...
  function addEvidence() {
    ...
    const prefix = evForm.role === '원고' ? '갑' : '을';
  ...
  const allSignRows = [
    { 구분: '소송문서', 서류명: '준비서면', 파일명: `준비서면(${caseNo || '-'}).pdf`, 크기: '56.6 KB' },
    ...evidenceRows.map((r, i) => ({ 구분: '증거서류', 서류명: `${r.서류명} (갑 제${i + 1}호증)`, 파일명: r.파일명 || `${r.서류명}.pdf`, 크기: '41.1 KB' })),
            ...
            <>
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  ...
                  <span style={{ fontSize: 15, fontWeight: 700, color: '#111' }}>민사서류 - 준비서면</span>
                </div>
                <div style={{ display: 'flex', gap: 5 }}>
                  <button onClick={() => setOpen({ s1: true, s2: true, s3: true, s4: true, s5: true })} style={{ height: 26, padding: '0 10px', border: '1px solid #b8c4cc', borderRadius: 2, background: '#fff', color: '#555', fontSize: 11, cursor: 'pointer' }}>전체열기 ▼</button>
                  <button onClick={() => setOpen({ s1: false, s2: false, s3: false, s4: false, s5: false })} style={{ height: 26, padding: '0 10px', border: '1px solid #b8c4cc', borderRadius: 2, background: '#fff', color: '#555', fontSize: 11, cursor: 'pointer' }}>전체닫기 ▲</button>
                </div>
              </div>
              ...
              <div id="sec-s2" style={{ background: '#fff', border: '1px solid #d0d8e4', marginBottom: 5, borderRadius: 2 }}>
                <SecHd label={inputMode === 'direct' ? '② 변론내용' : '② 준비서면 입력'} open={open.s2} toggle={() => toggle('s2')} />
                {open.s2 && (
                  <div style={{ padding: '12px 14px 14px' }}>
                    {inputMode === 'direct' ? (
                      /* ── 직접입력 모드: Rich text editor ── */
                      <table style={{ width: '100%', borderCollapse: 'collapse', border: '1px solid #d0d8e4' }}>
                        <tbody>
                          <tr>
                            <th style={{ ...TH, verticalAlign: 'top', paddingTop: 11, width: 100 }}>변론내용 <span style={{ color: '#e53e3e' }}>*</span> <span style={{ fontSize: 10, color: '#888' }}>&#9432;</span></th>
                            <td style={{ padding: '10px 12px', verticalAlign: 'top' }}>
                              {/* 에디터 박스 */}
                              <div style={{ border: '1px solid #c8cdd6', borderRadius: '2px' }}>
                                {/* 툴바 행 1 */}
                                ...
                                <div
                                  ...
                                  onInput={() => { if (causeEditorRef.current) setBriefContent(causeEditorRef.current.innerText); }}
                                  style={{ minHeight: 200, padding: '9px 12px', fontSize: 12, fontFamily: "'맑은 고딕',sans-serif", lineHeight: 1.8, outline: 'none', background: '#fff', color: '#222' }}
                                  data-placeholder="변론내용을 입력하세요. (한글 2000자 이내, 표나 그림은 파일첨부방식을 이용)"
                                />
                                <div style={{ background: '#f7f8fb', borderTop: '1px solid #e5e8ee', padding: '3px 10px', textAlign: 'right', fontSize: 11, color: '#888' }}>글자: {briefContent.length}/2000</div>
                              </div>
                      ...
                      <table style={{ width: '100%', borderCollapse: 'collapse', border: '1px solid #d0d8e4' }}>
                        <tbody>
                          <tr style={{ borderBottom: '1px solid #d0d8e4' }}>
                            ...
                            <td style={TD}>
                              <span style={{ fontSize: 13, color: '#888' }}>준비서면</span>
                            </td>
                          </tr>
                          <tr style={{ borderBottom: '1px solid #d0d8e4' }}>
              ...
              <div id="sec-s3" style={{ background: '#fff', border: '1px solid #d0d8e4', marginBottom: 5, borderRadius: 2 }}>
                <SecHd label="③ 서류명의인" open={open.s3} toggle={() => toggle('s3')} />
                {open.s3 && (
                  <div style={{ padding: '12px 14px 14px' }}>
                    <table style={{ width: '100%', borderCollapse: 'collapse', border: '1px solid #d0d8e4' }}>
                      <thead>
              ...
              <div id="sec-s4" style={{ background: '#fff', border: '1px solid #d0d8e4', marginBottom: 5, borderRadius: 2 }}>
                <SecHd label="④ 입증서류" open={open.s4} toggle={() => toggle('s4')} />
                {open.s4 && (
              ...
              <div id="sec-s5" style={{ background: '#fff', border: '1px solid #d0d8e4', marginBottom: 5, borderRadius: 2 }}>
                <SecHd label="⑤ 첨부서류" open={open.s5} toggle={() => toggle('s5')} />
                {open.s5 && (
                  <div style={{ padding: '12px 14px 16px' }}>
                    {/* 안내문 */}
                    <div style={{ fontSize: 11, color: '#333', lineHeight: 2, marginBottom: 10 }}>
                      <div>• 첨부서류로 제출한 문서는 증거로 사용될 수 없으며, 판결(결정) 등에 효력이 없습니다.</div>
                      <div>• 소송대리허가신청서 및 기타 신청서는 준비서면과 별도의 서류로 제출하여야 하므로 첨부서류에 포함되지 않도록 유의하여 주시기 바랍니다.</div>
                    </div>
              ...
              <div style={{ border: '1px solid #d8dce8', background: '#f8f9fb', borderRadius: 2, padding: '8px 12px', marginBottom: 10 }}>
                <ul style={{ listStyle: 'none', padding: 0, margin: 0 }}>
                  {['당사자 정보는 주민등록상 정보와 일치해야 하며, 허위 정보 입력 시 법적 책임이 발생할 수 있습니다.', '준비서면은 변론기일 전까지 제출하시기 바랍니다.'].map((t, i) => (
            ...
            <>
              ...
              <div style={{ display: 'flex', gap: 12 }}>
                ...
                <div style={{ width: 220, flexShrink: 0, background: '#fff', border: '1px solid #d0d8e4', borderRadius: 2, padding: '10px 12px' }}>
                  <div style={{ fontSize: 12, fontWeight: 700, color: TEAL, marginBottom: 8, cursor: 'pointer' }}>• 준비서면 📄</div>
                  {evidenceRows.map((r, i) => (
                    <div key={r.id} style={{ fontSize: 11, color: '#555', padding: '3px 0 3px 12px' }}>• [갑{i + 1}]{r.서류명} 📄</div>
                  ))}
                  {attachRows.map(r => (
                ...
                <div style={{ flex: 1, background: '#fff', border: '1px solid #d0d8e4', borderRadius: 2, padding: '40px 50px', minHeight: 600 }}>
                  {inputMode === 'file' && briefFileName ? (
                    <div style={{ textAlign: 'center', padding: '80px 20px' }}>
                      ...
                      <div style={{ fontSize: 12, color: '#888', marginTop: 8 }}>파일첨부방식으로 작성된 준비서면입니다.</div>
                    </div>
                  ) : (
                    <>
                      <div style={{ textAlign: 'center', fontSize: 22, fontWeight: 700, marginBottom: 30 }}>준 비 서 면</div>
                      ...
                      <div style={{ fontSize: 14, fontWeight: 700, textAlign: 'center', marginBottom: 12 }}>변 론 내 용</div>
                      <div style={{ fontSize: 13, lineHeight: 1.8, marginBottom: 24, whiteSpace: 'pre-wrap' }}>{briefContent || '(내용 없음)'}</div>

                      {evidenceRows.length > 0 && (
                        <>
                          <div style={{ fontSize: 14, fontWeight: 700, textAlign: 'center', marginBottom: 12 }}>입 증 방 법</div>
                          {evidenceRows.map((r, i) => (
                            <div key={r.id} style={{ fontSize: 13, lineHeight: 1.8 }}>갑 제{i + 1}호증:  {r.서류명}</div>
                          ))}
                      ...
                      {attachRows.length > 0 && (
                        <>
                          <div style={{ fontSize: 14, fontWeight: 700, textAlign: 'center', marginBottom: 12 }}>첨 부 서 류</div>
                          {attachRows.map((r, i) => (
                      ...
                      <div style={{ textAlign: 'center', fontSize: 13, marginBottom: 20 }}>{court} 귀중</div>
