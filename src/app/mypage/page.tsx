...
import { UnconfirmedDeliveryContent as UnconfirmedDeliveryPage, AllDeliveryContent as AllDeliveryPage } from '@/components/mypage/DeliveryPages'
...
export default function MyPage() {
  ...
  const ConfirmedCasesContent = () => {
    ...
    return (
      <div>
        <PageHd title="확정된사건" actions={<><ActBtn label="📌 나의 메뉴 추가" /><ActBtn label="🖨 출력" /></>} />

        {/* 필터 */}
        <div style={{ background:'#fff', borderBottom:'1px solid #eee', padding:'12px 14px', display:'flex', flexDirection:'column', gap:8 }}>
          <div style={{ display:'flex', gap:8, alignItems:'center', flexWrap:'wrap' }}>
            <span style={{ fontSize:12, fontWeight:600, color:'#555', minWidth:50 }}>소송유형</span>
            <select style={{ height:28, border:'1px solid #c8cdd6', borderRadius:3, fontSize:12, padding:'0 6px', fontFamily:'inherit' }}>
              {['전체','민사','형사','가사','행정','특허','회생파산'].map(t => <option key={t}>{t}</option>)}
            </select>
            <select style={{ height:28, border:'1px solid #c8cdd6', borderRadius:3, fontSize:12, padding:'0 6px', fontFamily:'inherit' }}>
              <option>전체</option>
            </select>
            <span style={{ fontSize:12, fontWeight:600, color:'#555', marginLeft:12 }}>법원</span>
            <select style={{ height:28, border:'1px solid #c8cdd6', borderRadius:3, fontSize:12, padding:'0 6px', fontFamily:'inherit', minWidth:100 }}>
              <option>전체</option>
            </select>
          </div>
          <div style={{ display:'flex', gap:8, alignItems:'center', flexWrap:'wrap' }}>
            <label style={{ fontSize:12, display:'flex', alignItems:'center', gap:4 }}>
              <input type="radio" name="cf-search" defaultChecked style={{ accentColor:'#003366' }} /> 확정일자
            </label>
            <label style={{ fontSize:12, display:'flex', alignItems:'center', gap:4 }}>
              <input type="radio" name="cf-search" style={{ accentColor:'#003366' }} /> 사건번호
            </label>
            <input type="date" style={{ height:28, border:'1px solid #c8cdd6', borderRadius:3, fontSize:12, padding:'0 6px' }} />
            <span>~</span>
            <input type="date" style={{ height:28, border:'1px solid #c8cdd6', borderRadius:3, fontSize:12, padding:'0 6px' }} />
            {['오늘','3일','1주일','1개월','전체'].map(b => (
              <button key={b} style={{ height:28, padding:'0 12px', border:'1px solid #c8cdd6', borderRadius:3, fontSize:12, background:'#fff', cursor:'pointer', color:'#555', fontFamily:'inherit' }}>{b}</button>
            ))}
          </div>
          <div style={{ display:'flex', gap:8, alignItems:'center' }}>
            <span style={{ fontSize:12, fontWeight:600, color:'#555', minWidth:50 }}>정렬순서</span>
            {['확정일자 ↓','법원 ↑','사건번호 ↓'].map(s => (
              <select key={s} style={{ height:28, border:'1px solid #c8cdd6', borderRadius:3, fontSize:12, padding:'0 6px', fontFamily:'inherit' }}>
                <option>{s}</option>
              </select>
            ))}
          </div>
          <div style={{ display:'flex', justifyContent:'center', marginTop:4 }}>
            <button style={{ height:32, padding:'0 36px', background:'#7a8a9e', color:'#fff', border:'none', borderRadius:3, fontSize:13, fontWeight:600, cursor:'pointer', fontFamily:'inherit' }}>조 회</button>
          </div>
        </div>
        ...
        {/* 페이지네이션 */}
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'10px 14px', background:'#fff', borderTop:'1px solid #eee' }}>
          <span style={{ fontSize:12, color:'#555' }}>총 <strong>{CONFIRMED_CASES.length}</strong>건</span>
          <div style={{ display:'flex', gap:2, alignItems:'center' }}>
            <button onClick={() => setPage(1)} disabled={page===1} style={{ height:26, width:28, border:'1px solid #ccc', borderRadius:2, background:'#fff', cursor:'pointer', fontSize:11 }}>«</button>
            <button onClick={() => setPage(Math.max(1,page-1))} disabled={page===1} style={{ height:26, width:28, border:'1px solid #ccc', borderRadius:2, background:'#fff', cursor:'pointer', fontSize:11 }}>‹</button>
            {Array.from({ length: totalPages }, (_, i) => i + 1).map(p => (
              <button key={p} onClick={() => setPage(p)} style={{ height:26, minWidth:28, padding:'0 6px', border: p===page ? 'none' : '1px solid #ccc', borderRadius:2, background: p===page ? '#003366' : '#fff', color: p===page ? '#fff' : '#555', cursor:'pointer', fontSize:12, fontWeight: p===page ? 700 : 400 }}>{p}</button>
            ))}
            <button onClick={() => setPage(Math.min(totalPages,page+1))} disabled={page===totalPages} style={{ height:26, width:28, border:'1px solid #ccc', borderRadius:2, background:'#fff', cursor:'pointer', fontSize:11 }}>›</button>
            <button onClick={() => setPage(totalPages)} disabled={page===totalPages} style={{ height:26, width:28, border:'1px solid #ccc', borderRadius:2, background:'#fff', cursor:'pointer', fontSize:11 }}>»</button>
          </div>
          <select style={{ height:26, border:'1px solid #ccc', borderRadius:3, fontSize:12, padding:'0 6px' }}>
  ...
  // ── 미확인송달문서 (DB연동) ────────────────────────────────────
  // ── 송달문서 (외부 컴포넌트) ──────────────────────────────
  // UnconfirmedDeliveryNewContent, AllDeliveryNewContent → DeliveryPages.tsx로 분리

  // ── 실습 전용 (소장/답변서/준비서면) ─────────────────────
  // ── 실습 전용: 사건 참고 정보 열람 + 서류제출 바로가기 ──
  const PracticeContent = ({ docType }: { docType: 'complaint' | 'answer' | 'brief' }) => {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const [pAssignments, setPAssignments] = useState<any[]>([])
    ...
    useEffect(() => {
      ...
      setPLoading(true)
      ;(async () => {
        const { data: assigns } = await supabase.from('case_assignments').select('*').eq('student_id', user.id).eq('doc_type', docType).order('assigned_at', { ascending: false })
        if (!assigns?.length) { setPAssignments([]); setPLoading(false); return }
        const caseIds = [...new Set(assigns.map((a: { case_id: string }) => a.case_id))]
        const { data: cases } = await supabase.from('practice_cases').select('*').in('id', caseIds)
        const caseMap = new Map((cases || []).map((c: { id: string }) => [c.id, c]))
        setPAssignments(assigns.map((a: { case_id: string }) => ({ ...a, case: caseMap.get(a.case_id) || {} })))
        setPLoading(false)
      })()
    // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [user, docType])

    const tgl = (aId: string, key: string) => setToggled(p => ({ ...p, [aId]: { ...(p[aId] || {}), [key]: !(p[aId]?.[key]) } }))
    ...
    const t = info[docType]

    const diffBadge = (d: string) => {
      const m: Record<string, { bg: string; color: string; label: string }> = { basic: { bg: '#e8f5e9', color: '#2e7d32', label: '기초' }, intermediate: { bg: '#fff3e0', color: '#e65100', label: '중급' }, advanced: { bg: '#fce4ec', color: '#c62828', label: '고급' } }
      const s = m[d] || m.basic
      return <span style={{ background: s.bg, color: s.color, padding: '2px 8px', borderRadius: 10, fontSize: 11, fontWeight: 700 }}>{s.label}</span>
    }

    const textBox: React.CSSProperties = { background: '#f8f9fb', border: '1px solid #e0e6ee', padding: 12, borderRadius: 2, fontSize: 12, lineHeight: 1.8, whiteSpace: 'pre-wrap', marginBottom: 8 }
    const toggleBtn: React.CSSProperties = { background: 'none', border: 'none', color: '#0098a3', cursor: 'pointer', fontSize: 12, fontWeight: 600, padding: '4px 0' }

    return (
      <div>
        <PageHd title={t.title} actions={<ActBtn label="🖨 출력" />} />

        {/* 진행 방법 안내 */}
        <div style={{ background: '#f0f9ff', border: '1px solid #0098a3', borderRadius: 4, padding: 16, marginBottom: 20 }}>
          <div style={{ fontSize: 13, fontWeight: 700, color: '#003366', marginBottom: 8 }}>📋 {t.title} 진행 방법</div>
          {t.steps.map((s, i) => <div key={i} style={{ fontSize: 12, color: '#555', lineHeight: 1.8 }}>{['1️⃣','2️⃣','3️⃣','4️⃣'][i]} {s}</div>)}
          <div style={{ fontSize: 11, color: '#888', marginTop: 6 }}>※ 서류 작성은 반드시 [서류제출] 메뉴에서 진행합니다.</div>
        </div>

        {/* 바로가기 버튼 */}
        <button onClick={() => router.push(t.linkBase)} style={{ background: '#1a3a6b', color: '#fff', padding: '14px 28px', borderRadius: 4, fontSize: 14, fontWeight: 700, border: 'none', cursor: 'pointer', display: 'inline-flex', alignItems: 'center', gap: 8, marginBottom: 24, fontFamily: 'inherit' }}>
          📄 {t.linkLabel} →
        </button>
        ...
        {pLoading ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            {pAssignments.map((a: { id: string; status: string; due_date: string | null; case_id: string; case: Record<string, string> }) => {
              const c = a.case || {}
              ...
              return (
                <div key={a.id} style={{ border: '1px solid #d0d8e4', borderRadius: 4, padding: 20, background: '#fff' }}>
                  {/* Header */}
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      {diffBadge(c.difficulty || 'basic')}
                      <span style={{ fontWeight: 700, color: '#003366' }}>{c.case_name}</span>
                      {dueDate && <span style={{ fontSize: 11, color: '#888' }}>마감: {dueDate}</span>}
                    </div>
                    {isSubmitted ? <span style={{ color: '#16a34a', fontWeight: 600, fontSize: 12 }}>✅ 제출완료</span> : <span style={{ color: '#e53e3e', fontWeight: 600, fontSize: 12 }}>○ 미제출</span>}
                  </div>

                  <div style={{ fontSize: 12, color: '#555', marginBottom: 4 }}>{c.court} {c.division}</div>
                  <div style={{ fontSize: 12, color: '#333', marginBottom: 12 }}>사건번호: <strong>{c.case_number}</strong></div>

                  {/* 원고/피고 정보 */}
                  <div style={{ borderTop: '1px solid #e8edf0', paddingTop: 12, marginBottom: 12 }}>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 12 }}>
                      <div style={{ background: '#f7f8fc', borderRadius: 4, padding: '10px 12px' }}>
                        <div style={{ fontSize: 10, color: '#888', marginBottom: 3 }}>👤 원고</div>
                        <div style={{ fontSize: 13, fontWeight: 600 }}>{c.plaintiff || '-'}</div>
                      </div>
                      <div style={{ background: '#f7f8fc', borderRadius: 4, padding: '10px 12px' }}>
                        <div style={{ fontSize: 10, color: '#888', marginBottom: 3 }}>👤 피고</div>
                        <div style={{ fontSize: 13, fontWeight: 600 }}>{c.defendant || '-'}</div>
                      </div>
                    </div>

                    {/* 토글 섹션들 */}
                    {c.sample_claim_purpose && (
                      <div style={{ marginBottom: 4 }}>
                        <button onClick={() => tgl(a.id, 'purpose')} style={toggleBtn}>📄 청구취지 {isOpen(a.id, 'purpose') ? '▲' : '▼'}</button>
                        {isOpen(a.id, 'purpose') && <div style={textBox}>{c.sample_claim_purpose}</div>}
                      </div>
                    )}
                    {c.sample_claim_reason && (
                      <div style={{ marginBottom: 4 }}>
                        <button onClick={() => tgl(a.id, 'reason')} style={toggleBtn}>📄 청구원인 {isOpen(a.id, 'reason') ? '▲' : '▼'}</button>
                        {isOpen(a.id, 'reason') && <div style={textBox}>{c.sample_claim_reason}</div>}
                      </div>
                    )}
                    {c.case_facts && (
                      <div style={{ marginBottom: 4 }}>
                        <button onClick={() => tgl(a.id, 'facts')} style={toggleBtn}>💡 사건 배경 {isOpen(a.id, 'facts') ? '▲' : '▼'}</button>
                        {isOpen(a.id, 'facts') && <div style={textBox}>{c.case_facts}</div>}
                      </div>
                    )}
                  </div>

                  {/* 바로가기 버튼 */}
                  <div style={{ borderTop: '1px solid #e8edf0', paddingTop: 12, display: 'flex', justifyContent: 'flex-end' }}>
                    <button onClick={() => router.push(`${t.linkBase}?caseId=${a.case_id || c.id}`)} style={{ height: 36, padding: '0 20px', background: '#1a3a6b', color: '#fff', border: 'none', borderRadius: 4, fontSize: 13, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit', display: 'flex', alignItems: 'center', gap: 6 }}>
                      📄 {t.linkLabel} →
                    </button>
                  </div>
                </div>
        ...
        <aside style={{ width: 200, flexShrink: 0, background: '#fff', border: '1px solid #d8dce8', borderRadius: 4, overflow: 'hidden', boxShadow: '0 1px 4px rgba(0,0,0,.06)' }}>
          ...
          {openGroups['나의사건관리'] && (
            <>
              ...
              <SbItem label="확정된사건" page="confirmed-cases" />
