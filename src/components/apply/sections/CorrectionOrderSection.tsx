...
export default function CorrectionOrderSection({ data, onChange }: CorrectionOrderSectionProps) {
  ...
  return (
    <div style={{ border: '1px solid #c8cdd6', borderRadius: 2, marginBottom: 10, background: '#fff' }}>
      <SecHd label="보정명령 정보" open={open} toggle={() => setOpen(!open)} />
      {open && (
        <div style={{ padding: '14px 16px' }}>
          <table style={tbl}>
            ...
            <tbody>
              <tr style={row}>
                <td style={{ ...TD, textAlign: 'center' }}>{(data.correctionOrderNo as string) || '-'}</td>
                <td style={{ ...TD, textAlign: 'center' }}>{(data.orderDate as string) || '-'}</td>
                <td style={{ ...TD, textAlign: 'center' }}>{(data.correctionDeadline as string) || '-'}</td>
              </tr>
            </tbody>
          </table>
