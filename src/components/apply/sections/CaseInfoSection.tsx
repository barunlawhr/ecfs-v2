...
export default function CaseInfoSection({ variant, data, onChange, readOnly }: CaseInfoSectionProps) {
  ...
  // 데이터가 이미 채워져 있으면 읽기전용으로 표시
  const hasData = !!(data.caseNo || data.court || data.plaintiff)
  const isReadOnly = readOnly || (variant === 'existing' && hasData)
  ...
  const valStyle: React.CSSProperties = { fontSize: 13, color: '#222', fontWeight: 400 }
        ...
        <table style={tbl}>
          <tbody>
            <tr style={row}>
              ...
              <td style={TD}>
                {isReadOnly
                  ? <span style={valStyle}>{(data.court as string) || '-'}</span>
                  : <select style={{ ...SEL, width: 260 }} value={(data.court as string) || ''} onChange={e => onChange({ court: e.target.value })}><option value="">선택</option>{COURTS.map(c => <option key={c} value={c}>{c}</option>)}</select>
                }
              </td>
              <td style={TH}>사건번호</td>
              <td style={TD}>
                {isReadOnly
                  ? <span style={valStyle}>{(data.caseNo as string) || '-'}</span>
                  : <input type="text" style={{ ...INP, width: 200 }} value={(data.caseNo as string) || ''} onChange={e => onChange({ caseNo: e.target.value })} placeholder="예: 2026가소226035" />
                }
              </td>
            </tr>
            <tr style={row}>
              ...
              <td style={TD}>
                {isReadOnly
                  ? <span style={valStyle}>{(data.division as string) || '-'}</span>
                  : <input type="text" style={{ ...INP, width: 200 }} value={(data.division as string) || ''} onChange={e => onChange({ division: e.target.value })} />
                }
              </td>
              <td style={TH}>사건명</td>
              <td style={TD}>
                {isReadOnly
                  ? <span style={valStyle}>{(data.caseName as string) || '-'}</span>
                  : <input type="text" style={{ ...INP, width: 200 }} value={(data.caseName as string) || ''} onChange={e => onChange({ caseName: e.target.value })} />
                }
              </td>
            </tr>
            <tr style={row}>
              ...
              <td style={TD}>
                {isReadOnly
                  ? <span style={valStyle}>{(data.plaintiff as string) || '-'}</span>
                  : <input type="text" style={{ ...INP, width: 200 }} value={(data.plaintiff as string) || ''} onChange={e => onChange({ plaintiff: e.target.value })} />
                }
              </td>
              <td style={TH}>피고</td>
              <td style={TD}>
                {isReadOnly
                  ? <span style={valStyle}>{(data.defendant as string) || '-'}</span>
                  : <input type="text" style={{ ...INP, width: 200 }} value={(data.defendant as string) || ''} onChange={e => onChange({ defendant: e.target.value })} />
                }
              </td>
            </tr>
          </tbody>
